...
if ( ! function_exists( 'rock_star_setup' ) ) :
	...
	function rock_star_setup() {
		...
		if ( 'dark' == $options['color_scheme'] ) {
			...
			$bg_defaults['default-image'] = get_template_directory_uri() . '/images/gallery/background-dark-1920x1080.jpg';
		}
		elseif ( 'light' == $options['color_scheme'] ) {
			$default_bg_color = rock_star_default_light_color_options();

			$bg_defaults['default-image'] = get_template_directory_uri() . '/images/gallery/background-light-1920x1080.jpg';
...
function rock_star_scripts() {
	 ...
	 */
	if ( 'dark' != $options['color_scheme'] ) {
		wp_enqueue_style( 'rock-star-'. $options['color_scheme'], get_template_directory_uri() . '/css/colors/'. $options['color_scheme'] .'.css', array(), null );
	}

	wp_enqueue_script( 'rock-star-navigation', get_template_directory_uri() . '/js/navigation.min.js', array(), '20120206', true );

	// Load the html5 shiv.
	wp_enqueue_script( 'rock-star--html5', get_template_directory_uri() . '/js/html5.min.js', array(), '3.7.3' );
	wp_script_add_data( 'rock-star-html5', 'conditional', 'lt IE 9' );

	wp_enqueue_script( 'rock-star-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.min.js', array(), '20130115', true );
	...
	if ( ! $options['disable_scrollup'] ) {
		wp_enqueue_script( 'rock-star-scrollup', get_template_directory_uri() . '/js/scrollup.min.js', array( 'jquery' ), '20072014', true  );
	}

	/**
	 * Enqueue custom script for Rock Star.
	 */
	wp_enqueue_script( 'rock-star-custom-scripts', get_template_directory_uri() . '/js/custom-scripts.min.js', array( 'jquery' ), null );
...
function rock_star_enqueue_metabox_scripts() {
    //Scripts
	wp_enqueue_script( 'rock-star-metabox', get_template_directory_uri() . '/js/metabox.min.js', array( 'jquery', 'jquery-ui-tabs' ), '2013-10-05' );

	//CSS Styles
	wp_enqueue_style( 'rock-star-metabox-tabs', get_template_directory_uri() . '/css/metabox-tabs.css' );
}
